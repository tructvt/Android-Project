...
void BinaryDatabaseConverter::convert_to_binary() {
	if (!database_writer.is_opened()) {
		...
		LOGE("Database writer is not opened");
		return;
	}

	cout << "Converting XML to binary database ..." << endl;
	LOGI("Converting XML to binary database ...");
	vector<RecordedSentence>& all_sentences =
			database_reader.get_all_sentences();
	...
	for (int i = 0; i < n_sentences; ++i) {
		...
		for (int j = 0; j < n_phrases; ++j) {
			...
			for (int k = 0; k < n_syllables; ++k) {
				...
				database_writer.write_string(
						syllable.get_initial_phoneme_type());
				database_writer.write_string(syllable.get_initial_phoneme());
				database_writer.write_string(
						syllable.get_middle_phoneme_type());
				database_writer.write_string(syllable.get_middle_phoneme());
				database_writer.write_string(
						syllable.get_nucleus_phoneme_type());
				...
				database_writer.write_omega(
						syllable.get_left_syllable_tone() + 1);
				database_writer.write_string(
						syllable.get_left_syllable_phoneme_type());
				database_writer.write_string(
						syllable.get_left_syllable_final_phoneme());

				database_writer.write_string(
						syllable.get_right_syllable_name());
				database_writer.write_omega(
						syllable.get_right_syllable_tone() + 1);
				database_writer.write_string(
						syllable.get_right_syllable_phoneme_type());
				database_writer.write_string(
						syllable.get_right_syllable_initial_phoneme());
			}
		}
	}

	cout << "Converted successfully. #Number distinct syllables = "
			<< distinct_syllables.size() << endl;
	LOGI("Converted successfully. #Number distinct syllables = %d",
			(int ) distinct_syllables.size());
	save_database_statistic(distinct_syllables, count_syllables,
			kDatabaseStatisticPath);
}

void BinaryDatabaseConverter::save_database_statistic(set<string> syllables,
		int count_syllables, string filename) {
	string raw_text = "Number syllables: " + to_string(count_syllables)
			+ "\nNumber distinct syllables: " + to_string(syllables.size())
			+ "\n\n";
	for (set<string>::iterator it = syllables.begin(); it != syllables.end();
			++it) {
