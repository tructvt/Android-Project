...
bool BinaryDatabaseReader::load_data() {
	if (!reader.is_opened()) {
		cerr << "Error loading recorded database description (binary) file"
				<< endl;
		LOGE("Error loading recorded database description (binary) file");
		return false;
	}

	cout << "Loading recorded database description (binary) file ..." << endl;
	LOGI("Loading recorded database description (binary) file ...");
	...
	for (int i = 0; i < n_sentences; ++i) {
		...
		for (int j = 0; j < n_phrases; ++j) {
			...
			for (int k = 0; k < n_syllables; ++k) {
				...
				syllable.set_right_syllable_initial_phoneme(
						reader.read_string());
	...
	cout << "Recorded database description (binary) loaded successfully: "
			<< all_sentences.size() << " sentences" << endl << endl;
	LOGI(
			"Recorded database description (binary) loaded successfully: %d   sentences",
			(int )all_sentences.size());
	return true;
}
