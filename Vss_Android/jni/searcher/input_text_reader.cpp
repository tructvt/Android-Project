...
bool InputTextReader::read_input_text_file(string file_name) {
	if (!read_text_file(file_name, raw_input_text)) {
		...
		LOGE("Cannot read input text file");
		return false;
	}

	if (debug_input_text_reader)
		cout << "Raw input text = " + raw_input_text << endl;
	LOGI("Raw input text = %s", raw_input_text.c_str());

	if (!parse_input_text()) {
		cerr << "Error parsing input text" << endl;
		LOGE("Error parsing input text");
		return false;
	}
...
void InputTextReader::set_input_text(string s) {
	raw_input_text = s;
	parse_input_text();
}
bool InputTextReader::parse_input_text() {
	...
	for (int i = 0; i <= number_tokens; ++i) {
		if ((i == number_tokens) || (is_punctuation(tokens[i]))) {
			if (!sentence_empty) {
				...
				searching_phrase.set_phrase_content(
						(i == number_tokens || is_long_silence(tokens[i])) ?
								"SILS" : "SIL");
	...
	if (debug_input_text_reader) {
		cout << endl << "Total sentence = " << all_searching_sentences.size()
				<< endl;
		LOGI("\n Total sentence = %d", (int )all_searching_sentences.size());
		for (int i = 0; i < (int) all_searching_sentences.size(); ++i) {
			searching_sentence = all_searching_sentences[i];
			cout << "Sentence " << (i + 1) << " = "
					<< searching_sentence.get_sentence_content() << endl;
			LOGI("Sentence %d = %s", (i + 1),
					searching_sentence.get_sentence_content().c_str());
			...
			for (int j = 0; j < (int) searching_phrases.size(); ++j) {
				...
				cout << "  Phrase " << (j + 1) << " = "
						...
						<< searching_phrase.get_phrase_level() << endl;
				LOGI("Phrase %d = %s %d ", (j + 1),
						searching_phrase.get_phrase_content().c_str(),
						searching_phrase.get_phrase_level());
				if (searching_phrase.has_sub_phrase()) {
					...
					for (int k = 0; k < (int) sub_phrases.size(); ++k) {
						...
						cout << "    Sub-phrase " << (k + 1) << " = "
								<< sub_phrase.get_phrase_content() << " "
								<< sub_phrase.get_phrase_level() << endl;
						LOGI("    Sub-phrase %d %s %d", (k + 1),
								sub_phrase.get_phrase_content().c_str(),
								sub_phrase.get_phrase_level());
					}
				}
			}
		}
