...
void test_unary() {
	...
	for (int i = 0; i < n; ++i) {
		...
		if (x != a[i]) {
			...
			LOGE("FAILED: %d != %d", x, a[i]);
			return;
		} else {
			if (debug_test_binary_encoding)
				cout << "OK: " << x << " = " << a[i] << endl;
		}
	}
...
void test_string() {
	...
	int n = 8;
	string a[] = { "cháy tiệm may, cả khu phố náo loạn.",
			...
			"Nhận được tin báo, lực lượng phòng cháy chữa cháy quận ba",
			"sau, vụ hỏa hoạn đã", " hoàn toàn. Cơ quan chức",
...
void test_all() {
	...
	for (int run = 0; run < total; ++run) {
		...
		if (t == 0) {
			...
			if (t == 1)
				writer.write_unary(number);
			else if (t == 2)
				writer.write_gamma(number);
			else if (t == 3)
				writer.write_delta(number);
			else
				writer.write_omega(number);
		}
	}
