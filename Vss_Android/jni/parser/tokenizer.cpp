...
bool Tokenizer::load_configure() {
	...
	LOGI("Loading tokenizer configure ...");
	...
	if (!ifs.is_open()) {
		...
		LOGE("File %d  not found!", path.c_str());
		return false;
	}
	...
	while (!ifs.eof()) {
		...
		if (s.length() > 19
				&& s.substr(0, 19).compare("<seperate_character") == 0) {
	...
	for (int i = 0; i < (int) dicts.size(); i++) {
		...
		if (!ifs.is_open()) {
			cerr << "File " << kDictionaryPath << dicts[i] << " not found!"
					<< endl;
			LOGE("File  %s %s not found!", kDictionaryPath.c_str(),
					dicts[i].c_str());
			return false;
		}
		...
		LOGI("%s %d ", dicts[i].c_str(), (int )dic.size());
	}

	ifs.open((kDictionaryPath + separator).c_str());
	if (!ifs.is_open()) {
		cerr << "File " << kDictionaryPath << separator << " not found!"
				<< endl;
		LOGE("File  %s %s not found!", kDictionaryPath.c_str(),
				separator.c_str());
		return false;
